...
def get_stop_codon_results(codon_pos,position,genetic_code_table,query_id,sequence,results_stop_codon):	
	...
	position_stop_codon = []
	pos = position
	for new_position in range(pos,len(sequence),3):
...
def get_seq_new_frameshift(query_id,align_path,results_stop_codon,final_pos):
	position_list = []
	position = results_stop_codon[query_id][0]
	alignment_len = results_stop_codon[query_id][1]
	if (final_pos - position)%3 == 1:
		final_pos = final_pos -1
	elif (final_pos - position)%3 == 2:
		final_pos = final_pos -2
	for record in SeqIO.parse(align_path, "fasta"):
		for new_position in range(position,final_pos,3):
			sequence = str(record.seq).upper()
			codon = sequence[new_position] + sequence[new_position+1] + sequence[new_position+2]
			frequency_bases = Counter(codon)
			if "-" in frequency_bases.keys():
				if frequency_bases["-"] != 3:
					for base_pos in range(len(codon)):
						if codon[base_pos] == "-":
							indel_pos = str(new_position + base_pos)
							position_list.append(indel_pos)
	...
	if len(position_list) == 0:
		...
		results_stop_codon[query_id].append(", ".join(position_list))

	return results_stop_codon

#Save the results
def save_results(results_stop_codon,reference_info,output_dir):
	#Make a header
	header = ["ID", "Length_sequence","Frame_position", "Stop_codons","Position_stop_codons","Position_frameshift","Conclusion"]
	#
...
def alignment(fasta_path,output_path):
	...
	with open(outfile,"w") as out_handle:
		cmd = ["mafft", "--globalpair","--maxiterate","1000","--thread","1","--quiet","--preservecase", infile]
...
for info_alignment in set_alignment_files:
	...
	results_stop_codon = get_seq_new_frameshift(query_id,alignment,results_stop_codon,final_position)

#Save results
save_results(results_stop_codon,reference_info,output_dir)
